...
class TextAudioDataset(Dataset):
    def __init__(self, hparams, data_dir, metadata_path, rawdata_dir, train=True):
        super().__init__()
        self.hparams = hparams
        self.lang = Language(hparams.data.lang, hparams.data.text_cleaners, use_eos=True)
        self.train = train
    ...
    def __getitem__(self, idx):
        ...
        audio_path = os.path.join(self.rawdata_dir, spk_id, '{}.wav'.format(basename))
        ...
        duration = duration * self.hparams.audio.hop // self.hparams.window.scale  # need to be delete
...
def create_dataloader(hparams, cv):
    ...
    if cv == 0:
        trainset = TextAudioDataset(hparams, hparams.data.train_dir, hparams.data.train_meta,
                                    hparams.data.rawdata_dir, train=True)
        ...
        valset = TextAudioDataset(hparams, hparams.data.val_dir, hparams.data.val_meta,
                                  hparams.data.rawdata_dir, train=False)
